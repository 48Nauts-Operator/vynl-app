...
export async function POST(request: NextRequest) {
  try {
    ...
    // Match by name to avoid duplicates on repeated imports
    ...
    for (const pl of playlists) {
      ...
      const name = f(pl, "name", "name") as string;
      const isAutoGen = f(pl, "isAutoGenerated", "is_auto_generated");

      // Check if playlist with same name already exists
      const existing = sqlite.prepare(`SELECT id FROM playlists WHERE name = ?`).get(name) as { id: number } | undefined;
      if (existing) {
        playlistIdMap.set(srcId, existing.id);
        continue;
      }

      const result = sqlite.prepare(
        `INSERT INTO playlists (name, description, cover_path, is_auto_generated, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?)`
      ).run(
        name,
